...
def get_png_paths(pdf):
    pdf_path = Path(pdf)
    png_paths = (
        pdf_path.parent.parent
        / pdf_path.parent.stem.replace("pdf", "png")
        / pdf_path.stem
    ).glob("*.png")
    return sorted([str(path) for path in png_paths])
...
class LazyDataset(Dataset):
    ...
    def __init__(self, pdf, prepare: Callable, pages: Optional[List[int]] = None):
        ...
        self.init_fn = partial(get_png_paths, pdf)
        self.dataset = None
...
class LazyImageDataset(Dataset):
    ...
    def __init__(self, png_paths: List, prepare: Callable):
        super().__init__()
        self.prepare = prepare
        # self.folder_path = folder_path
        # self.init_fn = partial(rasterize_paper, pdf, pages=pages)
        self.png_paths = png_paths
        # self.png_paths = list(folder_path.glob("*/*.png"))
        self.dataset = None
        self.size = len(self.png_paths)
    ...
    def __getitem__(self, i):
        if i == 0 or self.dataset is None:
            self.dataset = ImageDataset(self.png_paths, self.prepare)
        if i <= self.size and i >= 0:
            return self.dataset[i], str(self.png_paths[i])
        else:
            raise IndexError
